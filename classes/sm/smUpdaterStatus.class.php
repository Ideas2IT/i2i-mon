...
class smUpdaterStatus extends smCore {
	...
	public function update($max_runs = 2) {
		switch($this->server['type']) {
			...
			case 'website':
				...
				break;
			case 'SecureSite':
				$result = $this->updateSecureSite($max_runs);
				break;
		}
		return $result;
	...
	protected function updateSecureSite($max_runs, $run = 1)
	{
		// save response time
		$time = explode(' ', microtime());
		$starttime = $time[1] + $time[0];
		...
		$options = array(
				...
				CURLOPT_NOBODY		   => true,		// stop getting body content
				CURLOPT_CERTINFO	   => false,
				CURLOPT_SSL_VERIFYHOST => false,
				CURLOPT_FRESH_CONNECT  => true
		);
	
		$ch      = curl_init( $this->server['ip'] );
		...
		if($status_code == '200') {
			...
			$this->status_new = 'off';
			$this->server['error'] = $this->error = $status_code . ' ' . $err . ' ' . $errmsg;
		}
		
		// check if server is available and rerun if asked.
		if($this->status_new == 'off' && $run < $max_runs) {
			return $this->updateSecureSite($max_runs, $run + 1);
		}
	...
	protected function notifyByEmail() {
		...
		//$mail->AddReplyTo(PJConfig::$AddReplyTo, PJConfig::$AddReplyToName);
		...
		if(sm_get_conf('log_email')) {
			// save to log
			sm_add_log($this->server['server_id'], 'email', sm_parse_msg($this->status_new, 'message', $this->server), implode(',', $userlist));
		}
	}
